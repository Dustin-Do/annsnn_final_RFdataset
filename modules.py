...
# Define an activation module Spiking Normalization
class SpikingNorm(nn.Module):
    # 'self.' represents the instance of the class. By using the 'self.'  we can access the attributes and methods of
    ...
    def __init__(self, momentum=0.1, scale=True, sigmoid=True ,eps=1e-6):
        ...
        self.lock_max = False
        if scale:
            # Add 'scale' as parameter of the module
            self.scale = Parameter(torch.Tensor([1.0]))
        else:
            # 'register_buffer' adds a buffer to the module.  This is typically used to register a buffer that should
            # not to be considered as a model parameter.
            self.register_buffer('scale', torch.ones(1))
        if self.sigmoid:
            self.scale.data *= 10.0
        self.momentum = momentum
        # 'torch.ones()' returns a tensor filled with the scalar value 1, with the shape defined by the variable
        # argument size.
        # In this case 'running_max' is registered as a single number 1
        self.register_buffer('running_max', torch.ones(1))

    def calc_scale(self):
        if self.sigmoid:
            # 'scale' equals to sigmoid function of 'self.scale'
            scale = torch.sigmoid(self.scale)
        else:
            # 'scale' equals to absolute value of 'self.scale'
    ...
    def forward(self, x):
        if self.training and (not self.lock_max):
            # 'F.relu(x)' applies ReLU
            # torch.max() return the max element of 'F.relu(x)
...
# Almost the same like in 'SpikingNorm', just different in 'calc_v_th' and 'extract_scale' functions
...
# Used to activate the class 'SpikingNorm' which replace 'ReLU'? Always find a way to set 'scale' equals true
...
# Used to clamp the ReLU node in the range[0,1]
...
# Used to replace normalized spiking node by integrate and fire node (non-normalized spiking node).
# 'neuron.IFNode' is defined function in library Spikingjelly
def replace_spikingnorm_by_ifnode(model):
    for name, module in model._modules.items():
        ...
        if module.__class__.__name__ == "SpikingNorm":
            # If 'v_reset != None', the voltage will be reset to 'v_reset when the neuron fires a pulse. If not,
            # the voltage will be subtracted from 'v_threshold'.
        ...
        if isinstance(module,SpikingNorm):
            if last_param_module is not None:
                if hasattr(last_param_module, 'weight'):
                    # 'extract_running_max' is defined in class SpikingNorm
